...
# --- Configuration ---
# INPUT is the already 480p-scaled MP4s
INPUT_DIR = "datasets/UAVHuman_480p_mp4/Action_Videos"
DATA_DIR = "datasets/UAVHuman_480p_16framed_gop"
...
# -------------------------------------------------------------------------
# Step 1: Force 16-frame GOP (.mp4 -> .mp4)
# -------------------------------------------------------------------------
...
def run_conversion():
    ...
    print(f"🎥 STEP 1: Scanning for pre-resized .mp4 videos in {INPUT_DIR}...")
    print(f"⚙️  Encoder: {mode}  |  Workers: {MAX_WORKERS}")
    video_files = glob.glob(os.path.join(INPUT_DIR, "*.mp4"))

    if not video_files:
        print("❌ No .mp4 videos found! Check your input directory.")
        return False

    print(f"⚡ Repackaging {len(video_files)} videos to enforce 16-frame GOPs using {MAX_WORKERS} worker(s)...")
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        results = list(tqdm(executor.map(process_video, video_files), total=len(video_files)))
